...
useEffect =(()=> {
    const categotyParams = params.category ? `&category=${params.category}`: "";
    fetch(`http://localhost:8000/top-headlines=${categotyParams}&pageSize=${pageSize}&API=${api_key}`)
    .then(response => {
        if(response.ok){
            throw new console.error('failled to fetch data');
    ...
    .then(json =>{
        setTotalResult(json.data.totalResult);
